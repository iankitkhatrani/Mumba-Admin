...
const ShopManagement = () => {
  ...
  const { ShopList,ShopDelete } = context
  
  useEffect( () => {
    const submitdata = async () => {
    
      setUserData(await ShopList())

  }

  console.log("cookies.get('name') ",cookies.get('name'))
  if( cookies.get('name') == "Shop"){
  ...
  const navigateToUserRegister = (userid) => {
    ...
    navigate('/ShopRegistration',{state:0});
  };
    ...
    <div className={styles['player-management-container']}>
      ...
      <div className={styles.filters}>
       ...
       <input
        ...
        <button className={styles.addbtn} onClick={ () => navigateToUserRegister() } >Add Shop</button>
